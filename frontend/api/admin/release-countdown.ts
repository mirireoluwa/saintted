...
import type { ReleaseCountdown } from "../lib/types";
import { DEFAULT_COUNTDOWN } from "../lib/types";
...
export default async function handler(
  ...
  const getCountdown = async (): Promise<ReleaseCountdown> => {
  ...
  if (req.method === "GET") {
    try {
      return res.status(200).json(await getCountdown());
    } catch (e) {
      console.error(e);
  ...
  if (req.method === "PATCH" || req.method === "PUT") {
    try {
      const body = req.body ?? {};
      const current = await getCountdown();
